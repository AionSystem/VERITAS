...
class CERTUSEngine {
  ...
  detectSpatialCluster(recentReports, radiusM = 2000, windowMs = 1200000, minReports = 5) {
    ...
    for (let i = 0; i < timeFiltered.length; i++) {
      ...
      if (cluster.length + 1 >= minReports) {
        const completeCount = cluster.filter(r => r.internalTier === 'Completely damaged').length;
        return {
          ...
          severity: completeCount >= minReports * 0.6 ? 'MASS_CASUALTY_RISK' : 'ELEVATED',
  ...
  async healthCheck() {
    ...
    if (this._supabaseClient) {
      try {
        await this._supabaseClient.from('health_check').select('*').limit(1);
        hc.supabase = 'healthy';
...
computePES(report, isRealModel = false) {
  ...
  if (isRealModel) {
    result.measurement_class = 'EVALUATIVE'; result.um_contribution = 0;
...
// v3.2.1 — CONSENT GATE SOFT SIGNAL
  ...
  // NOTE: consentGateResult is attached to the return object below.
  // Search for 'appeal_status' in score() return block and add this line before it:
  ...
  const dci_raw = parseFloat(Math.max(0, Math.min(1, norm.score)).toFixed(3));
  ...
  const seal = await this._sealResult({ dci: dci_raw, tier, dci_validity_status: um.validity_status }, reportUuid);
  this._logFCLEntry({
    _reportUuid: reportUuid,
    dci: dci_raw,
    tier,
  ...
  return {
    ...
    tier,
    usable: um.validity_status !== 'SUSPENDED' || hasValidHR,
    version,
...
routeToVersion(userId) {
  ...
  if (hash < this.PRODUCTION.canaryPercentage) return this.CANARY_VERSION = '3.2.1-canary';
  return this.VERSION;
}
...
async initialize(supabaseUrl = null, supabaseAnonKey = null, supabaseClient = null) {
  ...
  if (supabaseClient) {
    this.initSupabase(supabaseClient);
...
// ──────────────────────────────────────────────────────────────────────────
// calibrateWeights is already defined in Part 1. No need to duplicate.
